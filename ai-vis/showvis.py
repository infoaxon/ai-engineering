...
def load_data(path):
    ...
    df = (
        demographics[["recordId", "age"]]
        .merge(actions[["recordId", "numOfLogins30d"]], on="recordId", how="left")
        .merge(
            metrics[["recordId", "totalCustomerLifetimeValue"]],
            on="recordId",
            how="left",
        )
    )
...
def assign_segment(row, high_ltv, high_logins):
    if (
        row["totalCustomerLifetimeValue"] >= high_ltv
        and row["numOfLogins30d"] >= high_logins
    ):
        return "High-Value & Engaged"
    elif row["totalCustomerLifetimeValue"] < (high_ltv * 0.3) and row[
        "numOfLogins30d"
    ] < (high_logins * 0.5):
...
def main():
    ...
    if uploaded:
        ...
        for seg, group in df.groupby("segment"):
            ax.scatter(
                group["numOfLogins30d"], group["totalCustomerLifetimeValue"], label=seg
            )
