...
def display_embedding_insights(chunk_emb, chunks, num_examples=3):
    ...
    for i in range(min(num_examples, len(chunks))):
        ...
        print(f"  • Dimensionality: {len(vec)}")
        print(f"  • First 10 values: {vec[:10]}")
        norm = np.linalg.norm(vec)
        print(f"  • Vector norm: {norm:.4f}")
        # ASCII histogram of value distribution (10 bins)
        hist, edges = np.histogram(vec, bins=10)
        bars = "".join(f"|{h:3d}" for h in hist)
        print(f"  • Value histogram (10 bins):\n    {bars}")
        edges_str = ", ".join(f"{edges[j]:.2f}" for j in range(len(edges)))
        print(f"    bin edges: [{edges_str}]\n")
...
def main():
    ...
    # **Deep dive display**
    display_embedding_insights(chunk_emb, chunks, num_examples=3)
