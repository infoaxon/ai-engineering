...
if session.query(Customer).count() == 0:
    seed_data = [
        Customer(customer_id=101, name="Alice", age=35, location="Mumbai", last_login_days_ago=40,
                 active_policies=["Health", "Motor"], policy_expiring_in_days=15, missed_notifications=3,
                 email_engagement_score=0.2),
        Customer(customer_id=102, name="Bob", age=45, location="Delhi", last_login_days_ago=5,
                 active_policies=["Life"], policy_expiring_in_days=60, missed_notifications=0,
                 email_engagement_score=0.9),
        Customer(customer_id=103, name="Charlie", age=29, location="Bangalore", last_login_days_ago=70,
                 active_policies=["Motor"], policy_expiring_in_days=5, missed_notifications=5,
                 email_engagement_score=0.1),
        Customer(customer_id=104, name="Diana", age=52, location="Chennai", last_login_days_ago=15,
                 active_policies=["Health", "Home"], policy_expiring_in_days=90, missed_notifications=1,
...
def get_recommendations_from_model(customer: Customer):
    return get_recommendations_from_dict({
        ...
        "active_policies": customer.active_policies,
        "policy_expiring_in_days": customer.policy_expiring_in_days,
        "missed_notifications": customer.missed_notifications,
        "email_engagement_score": customer.email_engagement_score
    })
...
recommendations = get_recommendations_from_model(selected_customer)
if recommendations:
