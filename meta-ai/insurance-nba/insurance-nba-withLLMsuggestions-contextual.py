...
def get_recommendations(customer):
    ...
    if customer["policy_expiring_in_days"] <= 30:
        recs.append(
            (
                ...
                f"The policy is expiring in {customer['policy_expiring_in_days']} days.",
            )
        )
    ...
    if customer["last_login_days_ago"] > 30 or customer["missed_notifications"] > 2:
        recs.append(
            (
                "📬 You've missed some important updates. Catch up now!",
                f"Last login was {customer['last_login_days_ago']} days ago and missed {customer['missed_notifications']} notifications.",
            )
        )
    ...
    if customer["email_engagement_score"] < 0.3:
        recs.append(
            (
                "💡 Don't miss out! Check your email for exclusive offers.",
                f"Engagement score is low at {customer['email_engagement_score']}.",
            )
        )
    ...
    if "Motor" in policies and "Health" not in policies:
        recs.append(
            (
                "❤️ Secure your health with our affordable health plans.",
                "Customer has Motor insurance but no Health plan.",
            )
        )
    if "Health" in policies and "Life" not in policies:
        recs.append(
            (
                "🌟 Protect your family's future with a life insurance plan.",
                "Customer has Health plan but no Life insurance.",
            )
        )
