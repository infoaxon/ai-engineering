...
from .models import (
    APIConfig, AppConfig, CustomerConfig, EnvironmentConfig, Settings
)
...
class CustomerManager:
        ...
        for customer_id, customer_data in raw_config['customers'].items():
            if customer_data.get('active', True):
                customers[customer_id] = CustomerConfig(
                    ...
                    environments=customer_data.get('environments', ['dev', 'sit', 'uat', 'production']),
    ...
    def load_customer_config(self, customer_id: str) -> AppConfig | None:
        """Load API configuration for a specific customer from their Postman collection."""
        ...
        collection_path = self.base_dir / customer.postman_collection

        if not collection_path.exists():
            print(f"Warning: Postman collection not found for {customer_id}: {collection_path}")
            return None

        # Parse Postman collection
        apis = parse_postman_collection(collection_path)
        api_configs = [APIConfig(**api) for api in apis]
        ...
        for env_key in customer.environments:
            environments[env_key] = EnvironmentConfig(
                name=env_names.get(env_key, env_key.title()),
                apis=api_configs
            )
    ...
    def add_customer(
        ...
        Args:
            ...
            postman_collection: Path to the Postman collection file
    ...
    def update_customer(
        ...
        Args:
            ...
            postman_collection: New path to Postman collection (optional)
    ...
    def delete_customer(self, customer_id: str) -> bool:
        ...
        Also removes the associated collection file if it exists.
    ...
    def get_customer_apis(self, customer_id: str) -> list[dict]:
        """Get the list of APIs for a customer.
...
class ConfigManager:
    ...
    def config(self) -> AppConfig:
        ...
        current_mtime = self.config_path.stat().st_mtime if self.config_path.exists() else None
