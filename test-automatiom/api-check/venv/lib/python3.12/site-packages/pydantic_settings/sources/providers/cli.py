...
class _CliArg(BaseModel):
    ...
    def __init__(
        ...
        **values: Any,
        ...
        alias_path_dests = {
            f"{self.arg_prefix}{name}": index
            for name, index in self._alias_paths.items()
        }
    ...
    def get_enum_names(
        ...
        if annotation and _lenient_issubclass(annotation, Enum):
            enum_names += tuple(
                cls.get_kebab_case(val.name, kebab_case == "all") for val in annotation
            )
        return enum_names
    ...
    def is_parser_submodel(self) -> bool:
        return (
            not self.subcommand_dest
            and bool(self.sub_models)
            and not self.is_append_action
        )
...
class CliSettingsSource(EnvSettingsSource, Generic[T]):
    ...
    def _connect_root_parser(
        ...
        self._parser_map: defaultdict[
            str | FieldInfo, dict[int | None | str, _CliArg]
        ] = defaultdict(dict)
        ...
        for field_name, field_info in self._sort_arg_fields(model):
            ...
            if arg.subcommand_dest:
                for sub_model in arg.sub_models:
                    ...
                    subcommand_arg = self._parser_map[arg.subcommand_dest][
                        subcommand_alias
                    ]
                    ...
                    if self.cli_use_class_docs_for_groups:
                        subcommand_arg.kwargs["help"] = (
                            None
                            if sub_model.__doc__ is None
                            else dedent(sub_model.__doc__)
                        )
                ...
                self._convert_append_action(
                    arg.kwargs, field_info, arg.is_append_action
                )
    ...
    def _metavar_format_recurse(self, obj: Any) -> str:
        ...
        if is_union_origin(origin):
            ...
            return self._metavar_format_choices(
                [
                    _CliArg.get_kebab_case(val.name, self.cli_kebab_case == "all")
                    for val in obj
                ]
            )
    ...
    def _serialized_args(
        self, model: PydanticModel, _is_submodel: bool = False
    ) -> list[str]:
        alias_path_only_defaults: dict[str, Any] = {}
        ...
        for field_name, field_info in _get_model_fields(
            ...
            if arg.subcommand_dest:
                ...
                subcommand_args += self._serialized_args(
                    model_default, _is_submodel=True
                )
                continue
            if is_model_class(type(model_default)) or is_pydantic_dataclass(
                type(model_default)
            ):
                positional_args += self._serialized_args(
                    model_default, _is_submodel=True
                )
                continue
